...
class TestScroll:
    def test_scroll_vertical(self):
        ...
        svc = InputService.__new__(InputService)
        with patch.object(svc, "_human_move_to"), \
             patch("enikk.game.input.pyautogui") as mock_pag, \
             patch("enikk.game.input.time"), \
             patch("enikk.game.input.random") as mock_rand:
            mock_rand.uniform.return_value = 0.05
    ...
    def test_scroll_default_direction(self):
        ...
        with patch.object(svc, "_human_move_to"), \
             ...
             patch("enikk.game.input.random") as mock_rand:
            mock_rand.uniform.return_value = 0.05
            mock_rand.randint.return_value = 5
