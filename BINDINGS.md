...
所有绑定收敛到 `cu_perceive/config.py`：**环境变量覆盖 > 仓库布局推导**，
代码零硬编码（`C:\Users\jawn\...` 已从源码消失）。解析结果可用
`python -m cu_perceive config` 查看。
...
| P3 | Python 解释器 | `CU_PYTHON` | `bin/cu-perceive.sh` 用：env 优先，其次 PATH 探测 `python.exe` |
...
| P6 | apps.json | `CU_APPS_JSON` | 默认 `<CU_ROOT>/apps.json`；**用户级覆盖** `~/.config/cu-perceive/apps.json` 按 name 合并（`config.load_apps`） |
...
| `CU_PYTHON` | PATH 探测 | 仅 `bin/cu-perceive.sh` 用 |
| `CU_WSL_DISTRO` | `Ubuntu` | UNC 映射发行版（M3 前半） |

配套改动：`cu_perceive/__init__.py` 改 PEP 562 懒加载（重依赖 cv2/PIL 不再
